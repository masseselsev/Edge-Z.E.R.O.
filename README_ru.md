...
* **Высокоскоростная авто-загрузка:** Сервис `dnsmasq` отдаёт загрузчики `ipxe.efi` и `undionly.kpxe` по DHCP/TFTP, а отдачу ядра (`vmlinuz`) и ramdisk (`initrd.gz`) производит через **1 Gbps HTTP** (TCP) за считанные доли секунды.
...
* **Динамические шаблоны & Кэширующий Apt-Proxy:** Динамическая генерация `preseed.cfg` (Debian) и `user-data` (Ubuntu Subiquity) с проксированием всех `.deb` пакетов через встроенный `apt-cacher-ng` (порт `3142`) на локальной скорости LAN.
